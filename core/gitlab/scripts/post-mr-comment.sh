...
# Usage: ./post-mr-comment.sh <mr_iid> <comment_body>
...
if [ -z "$MR_IID" ] || [ -z "$COMMENT_BODY" ]; then
    echo "Error: MR IID and comment body are required"
    echo "Usage: ./post-mr-comment.sh <mr_iid> <comment_body>"
    exit 1
fi
